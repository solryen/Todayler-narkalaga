  ...
  if (featured) {
    return (
      <Link to={`/explore/${article.slug}`} className="group flex flex-col md:flex-row overflow-hidden bg-stone-100 hover:bg-[#FDFBF7] transition-colors duration-300">
         ...
         <div className="flex flex-col flex-1 p-8 md:p-12 justify-center border-l border-stone-200">
            ...
            <h3 className="font-serif text-3xl sm:text-4xl md:text-5xl leading-tight text-stone-900 mb-6 group-hover:text-orange-600 transition-colors">
              {article.title}
            </h3>
            {article.subtitle && (
  ...
  return (
    <Link to={`/explore/${article.slug}`} className="group flex flex-col overflow-hidden bg-stone-100 hover:bg-[#FDFBF7] transition-colors duration-300">
      ...
      <div className="flex flex-col flex-1 p-6 sm:p-8">
        ...
        <h3 className="font-serif text-2xl sm:text-3xl leading-tight text-stone-900 mb-4 group-hover:text-orange-600 transition-colors">
          {article.title}
        </h3>
        <time dateTime={article.published_at} className="mt-auto text-[10px] uppercase font-bold tracking-widest opacity-60">
          {dateStr}
