...
export function NewsletterInline() {
  ...
  return (
    <div className="bg-[#EAE7E0] border border-[#E5E1D8] p-8 sm:p-12 my-12">
      <div className="text-[10px] uppercase tracking-[0.2em] mb-4 text-orange-600 font-medium">Newsletter</div>
      <h3 className="font-serif text-3xl leading-tight text-stone-900 mb-6 italic">
      ...
      <form onSubmit={handleSubmit} className="mt-4 flex flex-col sm:flex-row gap-4">
        <input
          ...
          className="flex-1 rounded-none border border-stone-300 bg-stone-50 px-4 py-3 text-xs tracking-widest font-bold uppercase text-stone-900 placeholder-stone-400 focus:border-stone-900 focus:outline-none transition-colors"
        />
        <button
          type="submit"
          disabled={status === 'loading'}
          className="inline-flex justify-center items-center rounded-none bg-white px-8 py-3 text-[10px] uppercase tracking-widest font-bold text-stone-900 border border-stone-200 hover:bg-stone-50 transition-colors disabled:opacity-70"
        >
