...
export function SignUpPage() {
  ...
  const adminNote = isGreek
    ? 'Ο λογαριασμός διαχείρισης εξακολουθεί να ελέγχεται ξεχωριστά από το admin email.'
    : 'Admin access is still controlled separately by the configured admin email.';
        ...
        {recoveryModeActive ? (
          ...
          <form onSubmit={submitSignIn} className="space-y-4">
            ...
            <button
              ...
              className="w-full mt-4 bg-stone-900 text-stone-50 py-4 text-[10px] uppercase tracking-widest font-bold border border-stone-900 hover:bg-stone-800 transition-colors flex items-center justify-center gap-3 disabled:opacity-70"
            >
              {loading ? (isGreek ? 'Σύνδεση...' : 'Signing in...') : isGreek ? 'Σύνδεση' : 'Sign In'}
            </button>
          </form>
        ) : mode === 'signUp' ? (
